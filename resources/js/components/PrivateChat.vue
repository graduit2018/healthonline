  ...
  <v-layout row>
    ...
    <v-flex id="privateMessageBox" class="messages mb-5" xs9>
       <v-list>
           ...
           <v-layout
           :align-end="(user.id==message.admin.id)"
            column
           >
             <v-flex>
               <v-layout column>
                  <v-flex>
                    <span class="small font-italic">{{message.admin.name}}</span>
                  </v-flex>

                  <v-flex>
                    <v-chip
                      :color="(user.id!==message.admin.id)?'red':'green'"
                      text-color="white"
                    >
  ...
  export default {
    ...
    created(){
              ...
              .listen('PrivateMessageSent',(e)=>{
                  this.activeFriend=e.message.admin_id;
